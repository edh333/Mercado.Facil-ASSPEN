...
export const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        if (isAdmin) {
            ...
            // Lógica de Recuperação de Senha Segura
            if (!validateCPF(recoveryUserCpf.replace(/\D/g, ''))) throw new Error("CPF do Familiar inválido.");
            if (!validateCPF(recoveryPrisonerCpf.replace(/\D/g, ''))) throw new Error("CPF do Interno inválido.");
            ...
            setFormSuccess("Senha alterada com sucesso! Você já pode entrar.");
            setTimeout(() => {
                setActiveTab('login');
                setFormSuccess(null);
            }, 2500);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 font-sans bg-slate-200 text-slate-900 relative">
      ...
      <div className="w-full max-w-[440px] bg-white rounded-2xl shadow-2xl border border-slate-300 overflow-hidden relative">
        ...
        <div className="px-8 py-6 -mt-6 bg-white rounded-t-3xl relative z-20">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {isRecovery && (
                    <div className="space-y-3 animate-fadeIn">
                        ...
                        <div className="pt-2 border-t border-slate-200 mt-2">
                            <p className="text-xs font-bold text-slate-500 uppercase mb-2">Definir Nova Senha</p>
                            <SimpleInput icon={Lock} label="Nova Senha" value={newPassword} onChange={(e:any) => setNewPassword(e.target.value)} type="password" />
                            <div className="mt-3">
                                <SimpleInput icon={CheckCircle} label="Confirmar Senha" value={confirmNewPassword} onChange={(e:any) => setConfirmNewPassword(e.target.value)} type="password" />
                            </div>
                        </div>
                    </div>
                )}
                ...
                >
                    {isLoading ? (
                        ...
                        isAdmin ? 'Acessar Painel' : (isRegister ? 'Finalizar Cadastro' : (isRecovery ? 'Salvar Nova Senha' : 'Entrar'))
                    )}
                </button>
            ...
            <div className="mt-8 pt-4 border-t border-slate-300 text-center">
                {(isAdmin || isRecovery) ? (
                    <button type="button" onClick={() => setActiveTab('login')} className="text-xs font-bold text-slate-700 hover:text-black flex items-center justify-center gap-2 mx-auto transition-colors">
