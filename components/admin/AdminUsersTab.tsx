    ...
    <div className="animate-slideUp space-y-6 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-5 rounded-lg border border-slate-200 shadow-sm">
        ...
        <div className="flex bg-slate-100 rounded-lg p-1 border border-slate-200">
          <button
            ...
            className={`px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${mainTab === 'CARDS' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
          >
            <Users size={14} /> Cadastros
          </button>
          <button
            onClick={() => setMainTab('VINCULOS')}
            className={`px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${mainTab === 'VINCULOS' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
          >
            <Link2 size={14} /> Vínculos Prisionais
          </button>
        </div>
        {mainTab === 'CARDS' && (
          <div className="flex bg-slate-100 rounded-lg p-1 border border-slate-200 w-full md:w-auto overflow-x-auto custom-scrollbar">
            {(['ALL','PENDING','ACTIVE','SUSPENDED'] as const).map(f => (
              <button key={f} onClick={() => setStatusFilter(f)}
                className={`flex-1 md:flex-none px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${statusFilter === f ? 'bg-[#0f172a] text-white' : 'text-slate-500 hover:text-slate-900'}`}>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1 relative group">
          <div className="absolute left-4 top-1/2 -translate-y-1/2 bg-white p-2.5 rounded-lg border border-slate-200 group-focus-within:bg-slate-900 group-focus-within:border-slate-900 transition-all duration-300 z-10 shadow-sm">
            <Search className="text-slate-400 group-focus-within:text-white transition-colors" size={20} />
          </div>
          <input
            className="w-full pl-16 pr-6 py-4 bg-white border-2 border-slate-200 focus:border-emerald-500 rounded-lg outline-none font-black text-xs text-slate-900 transition-all placeholder:text-slate-400 uppercase tracking-widest shadow-sm"
        ...
        {mainTab === 'CARDS' && (
          <>
            <button onClick={() => setShowPasswords(!showPasswords)} className={`px-8 py-4 rounded-lg border-2 transition-all flex items-center justify-center gap-3 font-black text-[10px] uppercase tracking-widest shadow-sm ${showPasswords ? 'bg-emerald-600 border-emerald-600 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
              {showPasswords ? <><EyeOff size={18}/> Ocultar</> : <><Eye size={18}/> Senhas</>}
            </button>
            <button onClick={() => setShowBulkActions(!showBulkActions)} className={`px-6 py-4 rounded-lg border-2 transition-all flex items-center justify-center gap-3 font-black text-[10px] uppercase tracking-widest shadow-sm ${showBulkActions ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
                  ...
                  <tr key={u.id} onClick={() => setViewingUser(u)} className="hover:bg-slate-50 transition-colors group cursor-pointer">
                    <td className="p-5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg flex items-center justify-center font-black text-white text-sm flex-shrink-0"
                    ...
                    <td className="p-5">
                      <div className="flex items-center justify-center gap-2 flex-wrap" onClick={(e) => e.stopPropagation()}>
                        ...
                        <button
                          ...
                          className="p-2.5 bg-slate-100 text-slate-600 hover:bg-emerald-500 hover:text-white rounded-lg border border-slate-200 shadow-sm active:scale-95 transition-all"
                        ...
                        <button
                          ...
                          className="p-2.5 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-lg border border-blue-200 shadow-sm active:scale-95 transition-all"
                        ...
                        {canManageCredits && (
                          <button
                            ...
                            className="p-2.5 bg-emerald-50 text-emerald-600 hover:bg-[#1e293b] hover:text-white rounded-lg border border-emerald-200 shadow-sm active:scale-95 transition-all"
                        ...
                        {u.status !== 'suspended' ? (
                          <button
                            ...
                            className="p-2.5 bg-amber-50 text-amber-600 hover:bg-amber-500 hover:text-white rounded-lg border border-amber-200 shadow-sm active:scale-95 transition-all"
                        ...
                        {toggleExcepcionalFlag && (
                          <button
                            ...
                            className={`p-2.5 rounded-lg border shadow-sm active:scale-95 transition-all ${u.autorizacaoExcepcional ? 'bg-purple-100 text-purple-700 border-purple-300 hover:bg-purple-200' : 'bg-slate-100 text-slate-400 border-slate-200 hover:bg-purple-50 hover:text-purple-500'}`}
      ...
      {mainTab === 'CARDS' && (
        <>
          ...
          {selectedUsers.size > 0 && (
            <div className="flex flex-wrap items-center gap-3 p-4 bg-slate-900 rounded-lg text-white animate-slideDown">
              <span className="font-black text-sm uppercase tracking-wide">{selectedUsers.size} selecionado(s)</span>
              <div className="flex-1" />
              <button onClick={handleBulkApprove} className="px-4 py-2 bg-[#0f172a] hover:bg-[#1e293b] rounded-lg font-black text-xs uppercase flex items-center gap-2"><CheckCircle size={16}/> Aprovar</button>
              <button onClick={handleBulkCredit} className="px-4 py-2 bg-blue-500 hover:bg-blue-600 rounded-lg font-black text-xs uppercase flex items-center gap-2"><PlusCircle size={16}/> Creditar</button>
              <button onClick={handleBulkSuspend} className="px-4 py-2 bg-amber-500 hover:bg-amber-600 rounded-lg font-black text-xs uppercase flex items-center gap-2"><Ban size={16}/> Bloquear</button>
              <button onClick={() => { setSelectedUsers(new Set()); setShowBulkActions(false); }} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg font-black text-xs uppercase">Limpar</button>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {filteredUsers.length === 0 ? (
              <div className="col-span-full py-20 text-center bg-white rounded-lg border-4 border-dashed border-slate-200 text-slate-300">
                <Users size={64} className="mx-auto mb-4"/>
                <p className="font-black uppercase tracking-[0.2em]">Nenhum familiar encontrado</p>
              </div>
            ) : filteredUsers.map(u => (
              <div key={u.id} onClick={() => setViewingUser(u)} className={`bg-white p-6 rounded-lg shadow-sm border-2 hover:shadow-md transition-all group relative overflow-visible cursor-pointer ${showBulkActions && selectedUsers.has(u.id) ? 'border-[var(--primary-color)] ring-2 ring-[var(--primary-color)]/20' : 'border-slate-200'}`}>
                ...
                <div className="flex items-start gap-5 relative z-10">
                  {showBulkActions && (
                    <button onClick={(e) => { e.stopPropagation(); toggleUserSelection(u.id); }} className="flex-shrink-0 p-2 rounded-lg border-2 border-slate-200 hover:border-emerald-500 hover:bg-emerald-50 transition-all">
                      {selectedUsers.has(u.id) ? <CheckCircle className="text-emerald-500" size={24}/> : <Square className="text-slate-400" size={24}/>}
                    </button>
                  )}
                  <div className="w-16 h-16 rounded-lg flex items-center justify-center font-black text-2xl text-white shadow-lg flex-shrink-0 group-hover:rotate-6 transition-transform duration-500"
                    style={{ backgroundColor: u.status === 'active' ? '#10b981' : u.status === 'suspended' ? '#ef4444' : '#f59e0b' }}>
                    {(u.name || '?').charAt(0)}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    {showPasswords && (
                      <div className="mt-3 px-3 py-1.5 bg-amber-50 border border-amber-200 rounded-lg inline-flex items-center gap-2">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mt-6 pt-5 border-t border-slate-100 relative z-10">
                  <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => setViewingUser(u)} className="p-3 bg-slate-100 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg border border-slate-200 shadow-sm active:scale-95 transition-all" title="Ver Documentação"><FileText size={18}/></button>
                    <button onClick={() => downloadUserReport(u)} className="p-3 bg-blue-50 text-blue-500 hover:bg-blue-600 hover:text-white rounded-lg border border-blue-200 shadow-sm active:scale-95 transition-all" title="Extrato JSON"><BarChart2 size={18}/></button>
                    {/* Dropdown com mais ações */}
                    <div className="relative">
                      <button
                        onClick={() => setOpenDropdown(openDropdown === u.id ? null : u.id)}
                        className="p-3 bg-slate-100 text-slate-500 hover:text-slate-900 hover:bg-slate-200 rounded-lg border border-slate-200 shadow-sm active:scale-95 transition-all"
                        title="Mais Ações"
                      >
                      ...
                      {openDropdown === u.id && (
                        <>
                          ...
                          <div className="absolute right-0 top-full mt-1 z-50 bg-white rounded-lg shadow-md border border-slate-200 py-2 min-w-[200px] animate-scaleIn origin-top-right">
                            {canManageCredits && (
                  ...
                  <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    {u.status === 'pending' && (
                      <button onClick={() => approveUser(u.id)} className="bg-[#0f172a] text-white px-5 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-md flex items-center gap-2 hover:brightness-110 active:scale-95 transition-all">
                        <CheckCircle size={16}/> Aprovar
                      </button>
                    )}
                    {u.status !== 'suspended' ? (
                      <button onClick={() => suspendUser(u.id, true)} className="bg-red-500 text-white px-5 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-md flex items-center gap-2 hover:brightness-110 active:scale-95 transition-all">
                        <Ban size={16}/> Bloquear
                      </button>
                    ) : (
                      <button onClick={() => suspendUser(u.id, false)} className="px-5 py-2.5 bg-slate-100 text-slate-900 border border-slate-200 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all">
                        Desbloquear
                      </button>
                    )}
                    <button onClick={() => {
                      ...
                      if (confirm(aviso)) deleteUser(u?.id);
                    }} className="p-3 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all active:scale-95">
                      <Trash2 size={20}/>
                    </button>
                  </div>
                </div>
