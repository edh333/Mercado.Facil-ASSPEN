  ...
  return (
    <>
      ...
      >
        ...
        />
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-black/70 text-white text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-full backdrop-blur-sm pointer-events-none flex items-center gap-1.5 shadow-md whitespace-nowrap">
    ...
    >
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-7 space-y-8">
              <div className="bg-white p-8 rounded-lg border border-slate-200 shadow-md relative overflow-hidden group">
              ...
              <div className="space-y-4 pt-4">
                ...
                {transaction.status === 'pending' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <button
                        ...
                        className="py-5 bg-red-50 text-red-600 border-2 border-red-200 rounded-lg font-black text-[11px] uppercase tracking-widest hover:bg-red-100 active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50 touch-target"
                      >
                      ...
                      <button
                        ...
                        disabled={isApproving || isRejecting}
                        className="py-5 bg-[#0f172a] text-white rounded-lg font-black text-[11px] uppercase tracking-widest hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50 touch-target"
                      >
                ...
                <button
                    onClick={handlePrint}
                    className="w-full py-5 bg-slate-900 text-white rounded-lg font-black text-[11px] uppercase tracking-[0.2em] hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-3"
                >
                ...
                <button
                    ...
                    className="w-full py-5 bg-white border-2 border-slate-300 text-slate-700 rounded-lg font-black text-[11px] uppercase tracking-[0.2em] hover:bg-slate-50 active:scale-95 transition-all flex items-center justify-center gap-3"
                >
            ...
            <div className="lg:col-span-5 flex flex-col gap-6">
              ...
              <div className="flex-1 min-h-[400px] bg-white p-4 rounded-lg border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden shadow-md group relative">
                {proofSrc && proofSrc !== 'PENDENTE_UPLOAD_LOCAL_CACHE' ? (
                  (proofSrc).toLowerCase().includes('.pdf') || (proofSrc).toLowerCase().includes('pdf') ? (
                    <div className="w-full h-full relative">
                      ...
                      />
                      <div className="absolute bottom-4 right-4 bg-red-600 text-white text-[10px] px-3 py-1.5 rounded-lg font-black shadow-md uppercase tracking-widest animate-pulse">Preview PDF</div>
                    </div>
                  ) : (
                ...
                {proofSrc && proofSrc !== 'PENDENTE_UPLOAD_LOCAL_CACHE' && (
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none">
                         <span className="bg-white text-black px-6 py-3 rounded-lg font-black text-[10px] uppercase tracking-widest shadow-md">Clique para Expandir</span>
                    </div>
                )}
              </div>
              {proofSrc && proofSrc !== 'PENDENTE_UPLOAD_LOCAL_CACHE' && (
              ...
              {!proofSrc || proofSrc === 'PENDENTE_UPLOAD_LOCAL_CACHE' ? (
                <button
                  onClick={() => fileInputRef.current?.click()}
                  disabled={isAttaching}
                  className="w-full py-4 bg-amber-50 border-2 border-amber-200 text-amber-600 rounded-lg font-black text-[10px] uppercase tracking-[0.2em] hover:bg-amber-100 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {isAttaching ? <Loader2 size={16} className="animate-spin"/> : <FileText size={16}/>} Anexar Comprovante (Admin)
                </button>
              ) : null}
