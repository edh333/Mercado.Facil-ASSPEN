...
import { AlertTriangle, RefreshCw, XCircle, Lock, Eye, EyeOff, Loader2, CheckCircle, ShieldCheck } from 'lucide-react';
 ...
 * irreversíveis (estorno/cancelamento de venda):
 *  1) Resumo do pedido para conferência
 *  2) Motivo OBRIGATÓRIO (vai para a notificação do familiar e auditoria)
 *  3) Senha do admin (mestra) para liberar — segunda camada de segurança
 * O modal NÃO executa nada: apenas coleta e devolve o par (motivo, senha).
 */
      ...
      footer={
        <>
          ...
          <button
            ...
            className={`px-8 py-3 rounded-xl text-white font-black text-[10px] uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 shadow-lg disabled:opacity-40 disabled:pointer-events-none ${
              acao === 'cancelar'
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-2.5">
            ...
            <div className="flex justify-between items-center gap-3 flex-wrap">
              ...
              <span className="text-xs font-black text-slate-900 uppercase">
                {order.paymentMethod === 'WALLET' ? 'Carteira' : order.paymentMethod === 'CASH' ? 'Dinheiro' : order.paymentMethod === 'FIADO' ? 'Fiado' : 'PIX'}
              </span>
            </div>
            <div className="flex justify-between items-center gap-3 flex-wrap">
