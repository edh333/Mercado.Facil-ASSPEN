    ...
    <div className="space-y-6 animate-slideUp pb-2">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-lg border border-slate-200 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-[#10b981] rounded-lg text-white shadow-sm">
            <Sparkles size={22} />
          </div>
          <div>
        ...
        <button
          onClick={exportCsv}
          className="bg-emerald-600 text-white px-5 py-3 rounded-lg font-black uppercase text-[9px] tracking-[0.2em] hover:bg-emerald-700 shadow-sm flex items-center gap-2 transition-all active:scale-95"
        >
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-5 shadow-sm">
        <div className="flex flex-wrap items-center gap-2">
          {([
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-lg font-black text-[10px] uppercase tracking-wider transition-all ${periodo === p.id ? 'bg-emerald-600 text-white shadow-sm' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
            >
              {p.label}
            </button>
          ))}
        ...
        {periodo === 'custom' && (
          <div className="flex flex-wrap items-center gap-3 mt-4 animate-fadeIn">
            ...
            <input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)} className="px-4 py-2.5 bg-white border border-[var(--border-input)] focus:border-emerald-500 rounded-lg font-black text-xs text-slate-900 outline-none" />
            <span className="text-slate-400 font-black text-xs">até</span>
            <input type="date" value={customEnd} onChange={e => setCustomEnd(e.target.value)} className="px-4 py-2.5 bg-white border border-[var(--border-input)] focus:border-emerald-500 rounded-lg font-black text-xs text-slate-900 outline-none" />
          </div>
        )}
      </div>
        ...
        <div className="lg:col-span-5 bg-white rounded-lg border border-slate-200 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            ...
            <select
              ...
              className="px-3 py-2 bg-white rounded-lg font-black text-[10px] text-slate-600 uppercase border border-[var(--border-input)] focus:border-emerald-500 outline-none"
            >
