...
export const AdminOrdersTab: React.FC<AdminOrdersTabProps> = ({
  ...
  return (
    <div className="space-y-6 animate-slideUp pb-20">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 bg-[var(--bg-card)] p-8 rounded-lg border border-[var(--border-color)] shadow-sm relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full xl:w-auto relative z-10">
            ...
            <div className="flex bg-[var(--bg-main)] rounded-lg p-1.5 border border-[var(--border-color)] w-full md:w-auto overflow-x-auto custom-scrollbar gap-1">
                <button onClick={() => setStatusFilter('ALL')} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${statusFilter === 'ALL' ? 'bg-[var(--text-main)] text-[var(--bg-card)] shadow-lg' : 'text-[var(--text-muted)] hover:bg-slate-100'}`}>Tudo</button>
                <button onClick={() => setStatusFilter('PENDING')} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${statusFilter === 'PENDING' ? 'bg-amber-500 text-white shadow-lg ring-2 ring-amber-300' : 'text-amber-600 hover:bg-amber-100'}`}>Pendentes</button>
                <button onClick={() => setStatusFilter('PAID')} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${statusFilter === 'PAID' ? 'bg-emerald-600 text-white shadow-lg' : 'text-emerald-600 hover:bg-emerald-100'}`}>Concluidos</button>
                <button onClick={() => setStatusFilter('REFUNDED')} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${statusFilter === 'REFUNDED' ? 'bg-indigo-600 text-white shadow-lg' : 'text-indigo-600 hover:bg-indigo-100'}`}>Devolvidos</button>
            </div>

            {/* Date Filter */}
            <div className="flex bg-[var(--bg-main)] rounded-lg p-1 border border-[var(--border-color)] w-full md:w-auto overflow-x-auto custom-scrollbar gap-1 items-center">
            ...
            <button
                ...
                className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${sortOrder === 'newest' ? 'bg-[var(--text-main)] text-[var(--bg-card)]' : 'bg-[var(--bg-main)] text-[var(--text-muted)] border border-[var(--border-color)]'}`}
            >
                <Clock size={14}/> {sortOrder === 'newest' ? 'Mais Recentes' : 'Mais Antigas'}
            </button>

            {/* View Mode Toggle */}
            <div className="hidden sm:flex bg-[var(--bg-main)] rounded-lg p-1 border border-[var(--border-color)]">
            ...
            <button
                ...
                className="px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 bg-emerald-500 text-white hover:bg-emerald-600"
            >
      ...
      <div className="flex flex-wrap items-center gap-4 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg px-6 py-4 shadow-sm">
        ...
        {dateFilter === 'DAY' && specificDate && (
          <span className="text-[10px] font-black uppercase tracking-widest text-blue-600 bg-blue-50 border border-blue-200 px-3 py-1.5 rounded-lg">
      ...
      <div className="relative group">
        <div className="absolute left-6 top-1/2 -translate-y-1/2 bg-[var(--bg-card)] p-3 rounded-lg border border-[var(--border-color)] group-focus-within:border-emerald-500 transition-all duration-300 z-10 shadow-sm">
          <Search className="text-[var(--text-muted)] group-focus-within:text-emerald-600 transition-colors" size={22} />
        </div>
        <input
          className="w-full pl-24 pr-8 py-6 bg-[var(--bg-card)] border border-[var(--border-color)] focus:border-emerald-500 rounded-lg text-sm font-black text-[var(--text-main)] outline-none transition-all placeholder:text-[var(--text-muted)] placeholder:font-medium uppercase tracking-[0.2em] "
      ...
      {/* Orders Grid/List */}
      <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6' : 'bg-[var(--bg-card)] rounded-lg border border-[var(--border-color)] shadow-sm overflow-hidden'}>
        {filteredOrders.length === 0 ? (
          ...
          <div key={order.id} className={`transition-all group relative overflow-hidden ${viewMode === 'grid' ? 'bg-[var(--bg-card)] rounded-lg shadow-sm border border-[var(--border-color)] hover:shadow-xl hover:-translate-y-2 p-8 flex flex-col' : 'flex flex-col sm:flex-row sm:items-center p-8 gap-8 border-b border-[var(--border-color)] last:border-0 hover:bg-[var(--bg-main)]'}`}>

{/* Status Indicator (Grid Top) */}
            <div className="flex justify-between items-start mb-6">
              <div className="px-4 py-1.5 bg-[var(--bg-main)] rounded-lg border border-[var(--border-color)] ">
              ...
              <span className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-xl border ${getStatusColor(order.status)}`}>
            ...
            <div className="flex-1 space-y-6">
              ...
              <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-lg bg-[var(--bg-main)] border border-[var(--border-color)] flex items-center justify-center text-[var(--text-muted)] shrink-0 group-hover:bg-emerald-500/10 group-hover:text-emerald-600 transition-colors">
                      <UserCheck size={24}/>
                  </div>
              ...
              {/* Inmate Info */}
              <div className="bg-white p-5 rounded-lg border border-slate-200 group-hover:border-emerald-500/50 transition-all">
            ...
            <div className="mt-8 pt-6 border-t border-[var(--border-color)] relative z-10">
              <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
                ...
                <div className="flex space-x-2 flex-wrap gap-y-2">
                  <button onClick={() => setPrintOrder(order)} className="p-4 min-h-[44px] min-w-[44px] bg-[var(--bg-main)] text-[var(--text-muted)] hover:bg-slate-100 hover:text-[var(--text-main)] rounded-lg border border-[var(--border-color)] shadow-sm active:scale-95 transition-all flex items-center justify-center" title="Imprimir Cupom 80mm"><Printer size={22}/></button>
                  <button onClick={() => setViewingReceipt({ data: order, type: 'ORDER' })} className="p-4 min-h-[44px] min-w-[44px] bg-[var(--bg-main)] text-[var(--text-muted)] hover:bg-emerald-50 hover:text-emerald-600 rounded-lg border border-[var(--border-color)] shadow-sm active:scale-95 transition-all flex items-center justify-center" title="Ver Recibo Digital"><FileText size={22}/></button>
                  <button onClick={() => setSelectedOrderDetails(order)} className="px-6 py-4 min-h-[44px] bg-emerald-500 text-white font-black rounded-lg text-[10px] uppercase tracking-widest shadow-lg hover:bg-emerald-600 active:scale-95 transition-all flex items-center gap-2 touch-target">
              ...
              {order.status === 'Pendente' || order.status === 'pending_payment' || order.status === 'pending' ? (
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setSelectedOrderDetails(order)} className="py-3 min-h-[48px] bg-rose-500 text-white font-bold rounded-lg text-[10px] uppercase tracking-widest shadow-lg hover:bg-rose-600 active:scale-95 transition-all flex items-center justify-center gap-2 touch-target">
                    <XCircle size={18}/> Rejeitar
                  </button>
                  <button onClick={() => handleQuickApprove(order)} className="py-3 min-h-[48px] bg-emerald-600 text-white font-bold rounded-lg text-[10px] uppercase tracking-widest shadow-lg hover:bg-emerald-700 active:scale-95 transition-all flex items-center justify-center gap-2 touch-target">
      ...
      {loadMoreOrders && filteredOrders.length >= 50 && (
        <div className="flex justify-center mt-8 pb-10">
          <button
            ...
            className="px-10 py-4 bg-[var(--bg-card)] border border-[var(--border-color)] text-[var(--text-main)] font-black uppercase tracking-[0.2em] rounded-lg hover:bg-emerald-500 hover:text-white hover:border-emerald-500 transition-all shadow-sm active:scale-95"
          >
