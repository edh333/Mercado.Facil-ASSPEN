...
return (
    <div className="space-y-6 animate-slideUp pb-20">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-[var(--bg-card)] p-5 rounded-lg border border-[var(--border-color)] shadow-sm">
          ...
          <div className="bg-[var(--bg-card)] p-5 rounded-lg border border-[var(--border-color)] shadow-sm border-l-4 border-l-emerald-500">
          ...
          <div className="bg-[var(--bg-card)] p-5 rounded-lg border border-[var(--border-color)] shadow-sm border-l-4 border-l-red-500">
          ...
          <div className="bg-[var(--bg-card)] p-5 rounded-lg border border-[var(--border-color)] shadow-sm border-l-4 border-l-amber-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-[var(--bg-card)] p-6 rounded-lg border border-[var(--border-color)] shadow-sm">
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          <div className="flex items-center flex-1 md:min-w-[400px] bg-[var(--bg-card)] pl-4 pr-0 py-0 border border-[var(--border-input)] focus-within:border-emerald-500 rounded-lg transition-all">
          ...
          <select
            ...
            className="px-4 py-2.5 rounded-lg border border-[var(--border-input)] font-black text-[10px] uppercase tracking-widest bg-[var(--bg-card)] text-[var(--text-main)] font-semibold focus:border-emerald-500 outline-none"
          >
          ...
          <button onClick={() => { setEditingProduct(null); setShowProductModal(true); }} className="bg-emerald-500 text-white px-6 py-2 rounded-lg font-black text-[10px] uppercase tracking-widest shadow-xl flex items-center justify-center gap-2 hover:opacity-90 transition-all active:scale-95"><Plus size={18}/> Novo Produto</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* XML Import */}
          <div className="lg:col-span-8 bg-[var(--bg-card)] p-6 rounded-lg border-2 border-dashed border-[var(--border-color)] shadow-sm">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center">
              <label className="flex-1 w-full bg-[var(--bg-main)] border-2 border-dashed border-[var(--border-color)] p-4 rounded-lg flex items-center justify-center gap-3 cursor-pointer hover:bg-[var(--bg-main)] transition-all group">
              ...
              <div className="w-full md:w-28 shrink-0">
                ...
                <input
                  type="number"
                  min="0"
                  className="w-full p-4 bg-[var(--bg-card)] border border-[var(--border-input)] focus:border-emerald-500 rounded-lg text-sm font-bold text-[var(--text-main)] text-center outline-none transition-all"
                  value={margin}
                  onChange={e => setMargin(e.target.value)}
                  placeholder="0"
                />
              </div>
<button onClick={handleImportXML} disabled={!xmlFile} className={`w-full md:w-auto px-8 py-4 rounded-lg font-black text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${!xmlFile ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : 'bg-[#4f46e5] hover:bg-[#4338ca] text-white'}`}>
            ...
            <button onClick={() => { if(confirm('Zerar preço/custo de TODOS os produtos com valor absurdo (>R$ 100 mil)? Use após NFe corrompida dar preços gigantescos.')) void sanitizeCatalog(); }} className="mt-3 w-full md:w-auto px-4 py-2.5 rounded-lg font-black text-[10px] uppercase tracking-widest transition-all bg-amber-500/10 text-amber-600 border border-amber-500/30 hover:bg-amber-500 hover:text-white flex items-center justify-center gap-2">
          ...
          <div className="lg:col-span-4 grid grid-cols-2 gap-3">
              <button onClick={onPrintCatalog} className="bg-[var(--bg-card)] border border-[var(--border-color)] p-6 rounded-lg flex flex-col items-center justify-center gap-3 hover:shadow-xl transition-all active:scale-95 group">
                  <div className="p-3 bg-[var(--bg-main)] rounded-lg text-[var(--text-muted)] group-hover:text-emerald-500 transition-colors"><Printer size={20}/></div>
                  <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text-main)]">Imprimir</span>
              </button>
              <button onClick={mergeDuplicateProducts} className="bg-[var(--bg-card)] border border-[var(--border-color)] p-6 rounded-lg flex flex-col items-center justify-center gap-3 hover:shadow-xl transition-all active:scale-95 group">
                  <div className="p-3 bg-[var(--bg-main)] rounded-lg text-[var(--text-muted)] group-hover:text-emerald-500 transition-colors"><RefreshCw size={20}/></div>
                  <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text-main)]">Mesclar</span>
              </button>
              <button onClick={() => setConfirmarEstoque(true)} className="col-span-2 bg-red-500/5 border border-red-500/20 p-4 rounded-lg flex items-center justify-center gap-3 hover:bg-red-500 text-red-600 hover:text-white transition-all active:scale-95 group">
      ...
      <div className={viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6' : 'bg-[var(--bg-card)] rounded-lg border border-[var(--border-color)] shadow-sm overflow-hidden'}>
        {filteredProducts.length === 0 ? (
          ...
          <div key={product.id} className={`transition-all overflow-hidden ${viewMode === 'grid' ? 'bg-[var(--bg-card)] rounded-lg shadow-sm border border-[var(--border-color)] hover:shadow-2xl hover:-translate-y-2 group flex flex-col relative' : 'flex flex-col sm:flex-row sm:items-center p-6 gap-6 border-b border-[var(--border-color)] last:border-0 hover:bg-[var(--bg-main)] relative'}`}>
{/* Estoque Crítico - Borda de alerta */}
              {product?.stock > 0 && product?.stock <= (product.minStock || 5) && (
                <div className="absolute inset-0 rounded-lg border-2 border-amber-400 pointer-events-none"></div>
              )}
              {(product?.stock || 0) <= 0 && (
                <div className="absolute inset-0 rounded-lg border-2 border-red-500 pointer-events-none"></div>
              )}

             {/* Thumbnail */}
             <div className={`${viewMode === 'grid' ? 'aspect-square relative overflow-hidden' : 'w-20 h-20 rounded-lg flex-shrink-0 relative overflow-hidden bg-[var(--bg-main)]'}`}>
                ...
                {product.available === false && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                        <span className="text-[10px] font-black text-white uppercase tracking-[0.2em] -rotate-12 border-2 border-white/30 px-3 py-1 rounded-lg shadow-2xl">Pausado</span>
                    </div>
                )}
                {(product?.stock || 0) <= 0 && product?.available !== false && (
                    <div className="absolute top-4 right-4 bg-red-600 text-white text-[10px] font-black px-3 py-1.5 rounded-lg shadow-lg uppercase tracking-widest animate-pulse">Esgotado</div>
                )}
             </div>
...
<div className="flex-1 min-w-0">
                ...
                <div className={`flex items-center justify-between gap-6 ${viewMode === 'grid' ? 'mt-6 pt-6 border-t border-[var(--border-color)]' : 'w-full sm:w-auto sm:min-w-[300px]'}`}>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => { setEditingProduct(product); setShowProductModal(true); }} className="p-3 bg-[var(--bg-main)] text-[var(--text-muted)] hover:text-emerald-500 rounded-lg border border-[var(--border-color)] shadow-sm active:scale-95 transition-all"><Edit size={18}/></button>
                      <button onClick={() => { if(confirm(`DESATIVAR ${product?.name || 'este produto'}? Ele deixará de aparecer nas vendas e no catálogo.`)) deleteProduct(product?.id); }} className="p-3 bg-red-500/5 text-red-500 hover:bg-red-600 hover:text-white rounded-lg border border-red-500/20 shadow-sm active:scale-95 transition-all"><Trash2 size={18}/></button>
                    </div>
                </div>
             </div>
          </div>
      ...
      {loadMoreProducts && productsLimit && filteredProducts.length >= productsLimit && (
        <div className="flex justify-center mt-12 pb-10">
          <button
            ...
            className="px-10 py-4 bg-[var(--bg-card)] border-2 border-[var(--border-color)] text-[var(--text-main)] font-black uppercase tracking-[0.2em] rounded-lg hover:bg-emerald-500 hover:text-white hover:border-emerald-500 transition-all shadow-sm active:scale-95 flex items-center gap-3"
          >
