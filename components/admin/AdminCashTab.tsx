...
const Stat: React.FC<{ label: string; value: string; color?: string; icon?: React.ReactNode }> = ({
  ...
  <div className="bg-white border border-slate-200 rounded-lg p-4 flex items-center gap-3 shadow-sm hover:shadow-md transition-shadow">
...
export const AdminCashTab: React.FC<AdminCashTabProps> = ({
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      showToast('Erro ao carregar sess�o de caixa.', 'error');
    } finally {
      setLoading(false);
    }
  }, [operatorId]);
  ...
  const handleOpen = async () => {
    ...
    // do HTML n�o impede digita��o de "-50".
  ...
  const handleClose = async () => {
    ...
    // Espelha a guarda do servidor: contagem f�sica negativa/NaN n�o existe.
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed top-4 right-4 z-50 px-5 py-3 rounded-lg shadow-lg text-white text-sm font-semibold transition-all ${toast.type === 'success' ? 'bg-[var(--primary-color)]' : 'bg-red-500'}`}>
          {toast.msg}
        </div>
      )}
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={reload} className="p-2 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-100 transition-all">
          <RefreshCw size={18} />
        </button>
      </div>
      ...
      {closeResult && (
        <div className={`mb-5 p-4 rounded-lg border flex items-start gap-3 ${closeResult.diff === 0 ? 'bg-[var(--primary-color)]/50 border-[var(--primary-color)]/20' : closeResult.diff < 0 ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'}`}>
          {closeResult.diff === 0
          ...
          <button onClick={() => handlePrint(closeSessionSnapshot)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-xs font-bold flex items-center gap-2 transition-all active:scale-95 shrink-0">
      ...
      {session ? (
        ...
        <>
          ...
          <div className="bg-[#0f172a] rounded-lg p-5 mb-6 text-white flex items-center justify-between shadow-sm">
            <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
            <button onClick={() => setModal('supplement')}
              className="flex flex-col items-center gap-1.5 p-4 bg-[var(--primary-color)]/50 hover:bg-[var(--primary-color)]/100 border border-[var(--primary-color)]/20 rounded-lg text-emerald-700 font-semibold text-sm transition-all active:scale-95">
              <Plus size={22} />
              Suprimento
            </button>
            <button onClick={() => setModal('withdrawal')}
              className="flex flex-col items-center gap-1.5 p-4 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg text-red-700 font-semibold text-sm transition-all active:scale-95">
              <Minus size={22} />
              Sangria
            </button>
            <button onClick={() => setModal('close')}
              className="flex flex-col items-center gap-1.5 p-4 bg-slate-100 hover:bg-slate-200 border border-slate-300 rounded-lg text-slate-700 font-semibold text-sm transition-all active:scale-95 col-span-2 sm:col-span-1">
              <LogOut size={22} />
              Fechar Caixa
            </button>
          </div>
          ...
          {((session.supplements || []).length > 0 || (session.withdrawals || []).length > 0) && (
            <div className="bg-white border border-slate-200 rounded-lg p-4 mb-4">
              <p className="text-sm font-bold text-slate-700 mb-3">Movimenta��es</p>
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-slate-500 text-sm mb-6 max-w-xs">Abra o caixa para come�ar a registrar as movimenta��es do dia.</p>
          <button
            onClick={() => setModal('open')}
            className="flex items-center gap-2 px-6 py-3 rounded-lg text-white font-bold shadow-lg hover:brightness-110 transition-all active:scale-95"
      ...
      {history.length > 0 && (
        <div className="mt-4">
          ...
          >
            <Clock size={16} />
            Hist�rico de Sess�es ({history.length})
          ...
          {expandHistory && (
            <div className="mt-3 space-y-2">
              {history.map(s => (
                <div key={s.id} className="bg-white border border-slate-200 rounded-lg p-4 flex items-center justify-between gap-4">
                  <div>
                  ...
                  {s.status === 'closed' && (
                    <button onClick={() => handlePrint(s)} className="p-2 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-all">
                      <Printer size={16} />
                    </button>
                  )}
                </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 px-4 pb-4 sm:pb-0" onClick={() => setModal(null)}>
          <div className="bg-white rounded-lg w-full max-w-sm p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            {modal === 'open' && (
              <>
                ...
                <input type="number" min="0" step="0.01" value={initialBalance}
                  ...
                  className="w-full border border-slate-300 rounded-lg px-4 py-3 text-lg font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-400 mb-4" />
              </>
            )}
            ...
            {(modal === 'supplement' || modal === 'withdrawal') && (
              <>
                <h3 className="text-lg font-bold text-slate-900 mb-1">
                  {modal === 'supplement' ? <><Plus size={16} className="text-[var(--primary-color)]" /> Suprimento</> : <><Minus size={16} className="text-red-500" /> Sangria de Seguran�a</>}
                </h3>
                ...
                <input type="number" min="0.01" step="0.01" value={amount}
                  ...
                  placeholder="0.00"
                  className="w-full border border-slate-300 rounded-lg px-4 py-3 text-lg font-bold text-slate-900 focus:outline-none focus:ring-2 mb-3" />
                ...
                <input type="text" value={reason}
                  ...
                  className="w-full border border-slate-300 rounded-lg px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:ring-2 mb-4" />
              </>
            )}
            ...
            {modal === 'close' && (
              <>
                ...
                <input type="number" min="0" step="0.01" value={closedBalance}
                  ...
                  placeholder="0.00"
                  className="w-full border border-slate-300 rounded-lg px-4 py-3 text-lg font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-red-400 mb-4" />
              </>
            )}
