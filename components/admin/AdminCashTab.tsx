...
const MovementRow: React.FC<{ m: CashMovement; type: 'in' | 'out' }> = ({ m, type }) => (
  <div className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
    ...
    <span className={`font-bold text-sm ${type === 'in' ? 'text-emerald-600' : 'text-red-600'}`}>
...
export const AdminCashTab: React.FC<AdminCashTabProps> = ({
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed top-4 right-4 z-50 px-5 py-3 rounded-2xl shadow-lg text-white text-sm font-semibold transition-all ${toast.type === 'success' ? 'bg-emerald-500' : 'bg-red-500'}`}>
          {toast.msg}
        </div>
      )}
      ...
      {closeResult && (
        <div className={`mb-5 p-4 rounded-2xl border flex items-start gap-3 ${closeResult.diff === 0 ? 'bg-emerald-50 border-emerald-200' : closeResult.diff < 0 ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'}`}>
          {closeResult.diff === 0
      ...
      {session ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
            ...
            <Stat label="Suprimentos" value={`+${fmt((session.supplements || []).reduce((a, s) => a + s.amount, 0))}`} color="text-emerald-600" icon={<TrendingUp size={20} className="text-emerald-400" />} />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
            <button onClick={() => setModal('supplement')}
              className="flex flex-col items-center gap-1.5 p-4 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-2xl text-emerald-700 font-semibold text-sm transition-all active:scale-95">
              <Plus size={22} />
              Suprimento
            </button>
                ...
                <div key={s.id} className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between gap-4">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    {s.status === 'closed' && (s.cashDifference ?? s.balanceDiff) !== undefined && (
                      <p className={`text-xs font-bold mt-1 ${(s.cashDifference ?? s.balanceDiff) === 0 ? 'text-emerald-600' : (s.cashDifference ?? s.balanceDiff) < 0 ? 'text-red-600' : 'text-amber-600'}`}>
