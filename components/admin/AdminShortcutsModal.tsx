...
export const AdminShortcutsModal: React.FC<AdminShortcutsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 animate-fadeIn" style={{ backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }} onClick={onClose}>
      <div className="bg-[var(--bg-card)] w-full max-w-2xl rounded-lg shadow-md flex flex-col max-h-[90vh] border border-[var(--border-color)] animate-slideUp" style={{ overflow: 'hidden' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 bg-[var(--text-main)] text-[var(--bg-card)] flex items-center gap-4 shrink-0">
          <div className="p-3 bg-[var(--bg-card)] text-[var(--text-main)] rounded-lg shadow-md">
            <Keyboard size={24}/>
          </div>
          <div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-2">
          {SHORTCUTS.map(s => (
            <div key={s.key} className="flex items-center justify-between gap-4 p-4 rounded-lg border border-[var(--border-color)] bg-[var(--bg-main)]/30 hover:bg-[var(--bg-main)]/60 transition-colors">
              <div className="flex items-center gap-3">
                <span className="px-3 py-1.5 rounded-lg bg-[#0f172a] text-white font-black text-xs font-mono shadow-md">{s.key}</span>
